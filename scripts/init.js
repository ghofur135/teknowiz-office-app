...
async function init() {
  const dbPath = path.join(process.cwd(), 'data', 'billing.sqlite3');
  const client = createClient({ url: `file:${dbPath}` });

  console.log('🔄 Menginisialisasi skema database WizBilling...');
  ...
  if (Number(compCheck.rows[0].count) === 0) {
    await client.execute({
      sql: `INSERT INTO company_profiles (
        ...
        is_pkp, tax_scheme, suket_pp55_number, tax_footer_note
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        ...
        0,
        'PP55_FINAL',
        'KET-01428/WPJ.10/KP.0403/2026',
        'PT Tekno Wiz Indonesia merupakan entitas PT Perorangan Wajib Pajak Badan Non-PKP (Memanfaatkan tarif PPh Final 0,5% sesuai PP No. 55/2022 jo. PP No. 20/2026).'
      ]
    });
    console.log('✅ Default company profile terpasang');
  }
  ...
  if (Number(userCheck.rows[0].count) === 0) {
    ...
    console.log('✅ Admin user resmi terpasang (dhimas@teknowiz.id)');
  }

  // Default Products
  const prodCheck = await client.execute('SELECT COUNT(*) as count FROM product_services');
  if (Number(prodCheck.rows[0].count) === 0) {
    ...
    console.log('✅ 6 Produk SaaS & 3 Layanan IT terpasang');
  }

  // Default Demo Client
  const cliCheck = await client.execute('SELECT COUNT(*) as count FROM clients');
  if (Number(cliCheck.rows[0].count) === 0) {
    await client.execute({
    ...
    console.log('✅ Demo client terpasang');
  }

  console.log('🎉 Inisialisasi skema & seed database selesai dengan sukses!');
}
